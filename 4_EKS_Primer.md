...
The Control plane determines when tasks are scheduled, and where they are routed to. The *Control Plane Nodes* consists of the control manager, Cloud controller, Scheduler, and API server.

- **Controller Manager**: runs background threads called *controllers* that detect and respond to cluster events.
...
**Data plane**: In Kubernetes, the Data plane is where the tasks are run. *Worker nodes* host the pods that are the components of the application workload:
